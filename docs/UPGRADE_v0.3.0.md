# Upgrade a v0.2.0 → v0.3.0 (Fase 9: extensibilidad)

Cambios que rompen compatibilidad y pasos de migración para quienes venían de
`mana-ciel` v0.2.0.

## ⚠️ Cambio de contrato de tool callable (breaking)

La firma oficial de una tool ahora es:
...
- Añade los parámetros keyword-only `tool_call_id` y `tenant_id` (puedes ignorarlos
  si no los necesitas, pero deben estar en la firma).
